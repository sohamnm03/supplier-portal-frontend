export default function Checkbox({ label, name, register, disabled, description, error }) {
  return (
    <div>
      <label className={`flex min-h-10 items-start gap-2.5 rounded-lg border border-slate-200 bg-slate-50/60 px-3 py-2.5 transition hover:border-slate-300 ${disabled ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}>
        <input type="checkbox" className="mt-0.5 size-4.5 shrink-0 rounded border-slate-300 accent-brand-600 focus:ring-brand-500" disabled={disabled} aria-invalid={Boolean(error)} aria-describedby={error ? `${name}-error` : undefined} {...register(name)} />
